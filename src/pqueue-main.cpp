...
void enqueueTime(int size);
void dequeueTime(int size);
...
void enqueueTime(int size)
{
	...
	for (int i = 0; i < size; i++) {
		values.push_back(randomString(RANDOM_STRING_LENGTH));
        priorities.push_back(randomInteger(1, size));
	}

	Timer tim;
	tim.start();
	for (int i = 0; i < 100; i++) {
        SLinkedPriorityQueue queue;
        for (int j = 0; j < size; j++) {
			queue.enqueue(values[j], priorities[j]);
		}
	}	
	tim.stop();
	cout << "This block took " << tim.elapsed() << " !" << endl;
 }

void dequeueTime(int N)
{
    /* Timer tim;
    tim.start();
    for (int i = 0; i < N; i++) {
    ...
    cout << "Deque block takes " << tim.elapsed() << "!" << endl;
    */
}
...
void test(T& queue) {
    ...
    while (true) {
        ...
        if (choice.empty() || choice == "Q") {
			...
			int size = getInteger("Number of elements: ");
            enqueueTime(size);
		}
		else if (choice == "C") {
